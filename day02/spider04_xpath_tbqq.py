...
from request.elasticsea import query_index, add_index, add_info
...
def parse(html):
    ...
    for li_element in li_elements:
        ...
        item['zhiye'] = li_element.xpath('.//div[@class="deanmadouzhiye"]//text()')[-1]
        city = li_element.xpath('.//div[@class="deanmadouinfos"]/div[5]/text()')[0]
        item['city'] = city.strip()[4:]

        height_weight_list = li_element.xpath('.//div[@class="deanmadouinfos"]/div[2]/div[1]//text()')
        item['height'] = height_weight_list[0]
        item['weight'] =  height_weight_list[2]

        item['star'] = li_element.xpath('.//div[@class="deanmadouinfos"]/div[2]/div[2]/span/text()')[0]
...
def save(item, index):
    ...
    add_info(index, 'madoinfo', document=item)
